...
def cross_truncate(
    ...
    bound_ = numpy.broadcast_to(
        numpy.asarray(bound, float).ravel(), (indices.shape[1],)
    )
    ...
    if numpy.any(bound_ == 0):
        ...
        if numpy.any(bound_):
            out &= cross_truncate(
                indices[:, bound_ != 0], bound_[bound_ != 0], norm=norm
            )
        return out
    ...
    if norm == 0:
        ...
        out = (
            numpy.sum((indices / bound_) ** norm, axis=-1) ** (1.0 / norm)
            <= 1 + nudge_factor
        )
