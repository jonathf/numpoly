...
@numba.njit
def loop_coeffs(
    expons1: numpy.typing.NDArray[numpy.uint32],
    coeffs1: Union[numpy.int64, numpy.typing.NDArray[numpy.uint32]],
    expons2: numpy.typing.NDArray[numpy.uint32],
    coeffs2: numpy.typing.NDArray[numpy.uint32],
    offset: int,
    where: numpy.typing.NDArray[numpy.bool_],
    names,
    values: numpy.typing.NDArray[numpy.float64],
):
    seen = list()
    for expon1, coeff1 in zip(expons1, coeffs1):
        for expon2, coeff2 in zip(expons2, coeffs2):
            key = (expon1 + expon2 + offset).ravel()
            key = "".join([chr(k) for k in key])
            idx = names.index(key)
            if key in seen:
                values[idx] += coeff1 * coeff2
            else:
                values[idx] = coeff1 * coeff2
            seen.append(key)
